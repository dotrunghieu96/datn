...
Item {
            ...
            Component.onCompleted: {
                if (playlist.rowCount() > 0)
                {
                    pathViewModel.setProperty(0, "artSrc",
                                              playlist.data(playlist.index(controller.nowPlayingIndex, 0), 260))
                    if (controller.nowPlayingIndex == playlist.rowCount() - 1)
                    {
                    ...
                    {
                        pathViewModel.setProperty(1, "artSrc",
                                                  playlist.data(playlist.index(controller.nowPlayingIndex + 1, 0), 260))
                    }

                    if (controller.nowPlayingIndex == 0)
                    {
    ...
    RoundButton {
        ...
        background: Image {
            ...
            source: {
                ...
                else if (controller.playbackMode === 2)
                {
                    if (repeatButton.pressed)
                    {
    ...
    TextField {
        ...
        text: controller.getTimeString(controller.position)
    ...
    TextField {
        ...
        text: controller.getTimeString(controller.duration)
        enabled: false
        background: Rectangle {
            anchors.fill: parent
            color: "transparent"
    ...
    Slider {
        ...
        onMoved: {
            controller.setPositionByPercent(timeSlider.position * 100)
        }
    }
}
